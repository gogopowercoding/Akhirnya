...
public class AdminDashboard extends JFrame {
    ...
    private void initComponents() {
        ...
        JButton pajakBtn = new JButton("Hitung Pajak"); // Tombol baru untuk GUI pajak
        JButton logoutBtn = new JButton("Logout");

        tambahBtn.addActionListener(e -> openTambahKendaraan());
        editBtn.addActionListener(e -> openEditKendaraan());
        pajakBtn.addActionListener(e -> openPajakGUI()); // Action listener untuk membuka GUI pajak
        logoutBtn.addActionListener(e -> logout());

        bottomPanel.add(tambahBtn);
        bottomPanel.add(editBtn);
        bottomPanel.add(pajakBtn); // Tambahkan tombol pajak di antara edit dan logout
    ...
    private void loadKendaraanData(String statusFilter) {
        try {
            ...
            model.setRowCount(0);
            int adminUserId = userId; // Gunakan user_id admin yang valid
            ...
            for (Object[] row : kendaraanList) {
                model.addRow(new Object[]{
                    ...
                    row[1], // Nomor Polisi
                    row[2], // Merk
                    row[7] != null ? row[7].toString().replace("_", " ") : "BELUM BAYAR" 
                });
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Gagal memuat data: " + e.getMessage(),
    ...
    private void openEditKendaraan() {
        try {
            int adminUserId = userId; // Gunakan user_id admin yang valid (sesuai dengan data di tabel users)
    ...
    private void openPajakGUI() {
        try {
            PajakKendaraanForm pajakGUI = new PajakKendaraanForm();
            pajakGUI.setVisible(true);
            pajakGUI.addWindowListener(new WindowAdapter() {
                @Override
                public void windowClosed(WindowEvent windowEvent) {
                    try {
                        loadKendaraanData((String) filterCombo.getSelectedItem()); // Segarkan tabel setelah GUI ditutup
                    } catch (Exception e) {
                        JOptionPane.showMessageDialog(AdminDashboard.this, "Gagal memuat ulang data: " + e.getMessage());
                    }
                }
            ...
            JOptionPane.showMessageDialog(this, "Gagal membuka GUI pajak: " + e.getMessage());
        }
    }
