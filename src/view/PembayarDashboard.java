...
public class PembayarDashboard extends JFrame {
    ...
    // Constructor yang menerima userId
    public PembayarDashboard(int userId) {
        this.userId = userId;
        initComponents();
    }

    private void initComponents() {
        setTitle("Dashboard Pembayar - User ID: " + userId);
        setSize(800, 600);
    ...
    try {
        ...
        if (conn == null) throw new Exception("Gagal mendapatkan koneksi database");

        RiwayatPembayaranView riwayatView = new RiwayatPembayaranView();
        riwayatView.setUserId(userId); // Penting! agar tombol "Kembali" tahu userId-nya

        RiwayatPembayaranController controller = new RiwayatPembayaranController(conn, riwayatView);
        controller.tampilkanRiwayatPembayaran(userId);
